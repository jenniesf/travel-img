...
module.exports = {
  ...
  getPost: async (req, res) => {
    try {
      ...
      const comment = await Comment.find({ post: req.params.id }); // all post's comments
     
      // put comment user IDs into array
      const commentOwnerIds = comment.map( element => element.user)
     
      // get comment owners usernames
      let commentUsername = []
      for (let i = 0; i < commentOwnerIds.length; i++){
        commentUsername.push(await User.findById(commentOwnerIds[i]));

      }

      // get post owner's username
      const postOwnerId = post.user
      const postUsername = await User.findById(postOwnerId);
